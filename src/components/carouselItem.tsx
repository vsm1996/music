...
const CarouselItem = ({ id, albumArtUrl, title, artist, releaseYear, genre }: Album) => {
  return (
    <div key={id} className="carousel-item group hover:drop-shadow-xl rounded-full carousel-center flex flex-col items-center p-40 bg-black cursor-pointer">
      <div className='grid grid-cols-1 justify-items-center items-center h-full'>
        <div className='col-start-1 row-start-1 bg-black opacity-55'>
          <Image
            ...
            height={275}
            alt={title}
            className="mask mask-circle z-0 group-hover:animate-[spin_3s_linear_infinite] min-w-full min-h-full transition object-contain object-center"
          />
        </div>
